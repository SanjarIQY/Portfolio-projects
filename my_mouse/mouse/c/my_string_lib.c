...
int* length_array_elements(char* string, char letter)
{
    ...
    int* ret_val = inner_work(number_of_str, string, letter);
    return ret_val;
}
...
{
    int number_of_str = number_of_string(input, letter);
    ...
    fill_array(input, letter, ret_val, &ind, length_of_elenments);
    ret_val[++ind] = NULL;
